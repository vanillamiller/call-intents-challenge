...
import { FIRST_PASS_FEW_SHOT, LIST_RESPONSE_FORMAT, SECOND_PASS_FEW_SHOT, THIRD_PASS_CLASSIFY } from "src/constants/prompts";
...
class IntentCategorizer {
        ...
        const secondPass = new TaskStep({
            ...
            returnPrompt: (map) => {
                ...
                const messages = chunks.map(chunk =>
                    chunk.reduce((acc, { id, parsedIntent }) => ({
                        ...acc,
                        [id]: parsedIntent
                    }), {}))
    ...
    public catagorizeIntents = async () => {
        ...
        return Object.values(this.intentsMap).map(({ id, intent, category }) => ({ id, intent, category }))
    }
}
